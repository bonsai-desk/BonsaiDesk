...
#region Render window
        ...
        private (ISeries, ISeries) AllFrameSeries()
        {
            switch (Series)
            {
                case ChartSeries.TotalBytes:
                    return (
                        GetByteSeriesByFrame("Inbound {0} B/s", NetworkDirection.Incoming),
                        GetByteSeriesByFrame("Outbound {0} B/s", NetworkDirection.Outgoing));

                case ChartSeries.MessageCount:
                    return (
                         GetCountSeriesByFrame("Inbound {0} Msg/s", NetworkDirection.Incoming),
                         GetCountSeriesByFrame("Outbound {0} Msg/s", NetworkDirection.Outgoing));

                case ChartSeries.Bandwidth:
                    return (
                        GetBandwidthSeriesByFrame("Inbound {0} B/s", NetworkDirection.Incoming),
                        GetBandwidthSeriesByFrame("Outbound {0} B/s", NetworkDirection.Outgoing));
        ...
        private (ISeries, ISeries) AllTicksSeries()
        {
            switch (Series)
            {
                case ChartSeries.TotalBytes:
                    return (
                        GetByteSeriesByIndex("Inbound {0} B/s", NetworkDirection.Incoming),
                        GetByteSeriesByIndex("Outbound {0} B/s", NetworkDirection.Outgoing));
                case ChartSeries.MessageCount:
                    return (
                        GetCountSeriesByIndex("Inbound {0} Msg/s", NetworkDirection.Incoming),
                        GetCountSeriesByIndex("Outbound {0} Msg/s", NetworkDirection.Outgoing));
                case ChartSeries.Bandwidth:
                    return (
                        GetBandwidthSeriesByIndex("Inbound {0} B/s", NetworkDirection.Incoming),
                        GetBandwidthSeriesByIndex("Outbound {0} B/s", NetworkDirection.Outgoing));
                default:
        ...
        {
            var data = networkProfiler.Ticks.Select(tick =>
                new DataPoint(tick.frameCount, tick.time,(float)tick.Count(direction)));

            // append the current frame
            var lastTick = Enumerable.Range(0, 1).Select(_ =>
            {
                 var tick = networkProfiler.CurrentTick();
                 return new DataPoint(tick.frameCount, tick.time, (float)tick.Count(direction));
            });
        ...
        {
            var data = networkProfiler.Ticks.Select(tick => new DataPoint(tick.frameCount, tick.time, (float)tick.Bytes(direction)));

            // append the current frame
            var lastTick = Enumerable.Range(0, 1).Select(_ =>
            {
                var tick = networkProfiler.CurrentTick();
                return new DataPoint(tick.frameCount, tick.time, (float)tick.Bytes(direction));
            });
        ...
        {
            var data = networkProfiler.Ticks.Select((tick, i) => new DataPoint(i, tick.time, (float)tick.Count(direction)));
        ...
        {
            var data = networkProfiler.Ticks.Select((tick, i) => new DataPoint(i, tick.time, (float)tick.Bytes(direction)));
        ...
        private ISeries GetBandwidthSeriesByIndex(string legend, NetworkDirection direction)
        {
            ...
            var data = allTicks
                ...
                .Select((tick, i) => new DataPoint(i - 1, tick.time, tick.value));

            return new Series(legend, data);
        }

        private DataPoint EstimateBandwidth(NetworkProfileTick tick, NetworkProfileTick prevTick, NetworkDirection direction)
        {
            ...
            if (prevTick.frameCount == 0 || tick.time <= prevTick.time)
                return new DataPoint(0, tick.time, estimatedBytes);
            
            return new DataPoint(0, tick.time, estimatedBytes / (tick.time - prevTick.time));
        }

        private ISeries GetBandwidthSeriesByFrame(string legend, NetworkDirection direction)
        {
            var data = networkProfiler.Ticks.Select(tick => new DataPoint(tick.frameCount, tick.time, tick.Bytes(direction) + tick.Count(direction) * EstimatedOverheadPerMessage));

            // append the current frame
            var lastTick = Enumerable.Range(0, 1).Select(_ =>
            {
                var tick = networkProfiler.CurrentTick();
                return new DataPoint(tick.frameCount, tick.time, tick.Bytes(direction) + tick.Count(direction) * EstimatedOverheadPerMessage);
            });
