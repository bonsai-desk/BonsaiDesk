...
public static class AssetStoreBatchMode
{
    ...
    public static void UploadAssetStorePackage(params string[] args)
    {
        ...
        {
            { "asset_store_output_path=",
                "The location to store the final package before upload",
                o => outputPath = o},
            { "asset_store_actually_upload",
                "Actually upload?",
                o => actuallyUpload = o != null},
        ...
        UploadAssetStorePackage(username, password, packageName, rootPath, mainAssets.ToArray(), loginTimeout, metadataTimeout, uploadTimeout, skipProjectSettings, actuallyUpload, outputPath);
    }
    /// <summary>
    ...
    static void Export(Package package, string localRootPath, string toPath, string outputPath)
    {
        File.Delete(toPath);
        ...
        if (outputPath != "")
        {
            AssetDatabase.ExportPackage(paths, outputPath);
        }
    }
